...
static void stack_init(void *arg)
{
    ...
    if (sys_thread_new("app_task", app_thread, &netif, APP_THREAD_STACKSIZE, APP_THREAD_PRIO+2) == NULL)
    {
...
void i2c_Task(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xQueueReceive(IMU_Queue, &imuData, pdMS_TO_TICKS(1)) == pdTRUE) {
            girosx = imuData.gyro_x;
